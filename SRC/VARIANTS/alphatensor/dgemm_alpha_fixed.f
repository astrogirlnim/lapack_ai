...
*
*     Initialize result matrix (apply beta scaling)
      DO I = 1, 4
          DO J = 1, 4
              C(I,J) = BETA * C(I,J)
          END DO
...
*     A_FLAT(1)→A(1,1), A_FLAT(9)→A(3,1), RESULT_FLAT(1)→C(1,1), RESULT_FLAT(9)→C(3,1)
      ...
      C(1,1) = C(1,1) + ALPHA * SCALAR_RESULT
...
*     A_FLAT(2,4)→A(1,2,4), B_FLAT(5)→B(2,1), Multiple C updates
      ...
      SCALAR_RESULT = A_CONTRIB * B_CONTRIB
      C(1,1) = C(1,1) + ALPHA * SCALAR_RESULT
      C(1,2) = C(1,2) - ALPHA * SCALAR_RESULT
      C(2,1) = C(2,1) + ALPHA * SCALAR_RESULT
      C(2,2) = C(2,2) - ALPHA * SCALAR_RESULT
*
...
*
      ...
      WRITE(LOG_UNIT,*) 'DIRECT2D: Using direct matrix access'
      WRITE(LOG_UNIT,*) 'DIRECT2D: No flattening required'
      WRITE(LOG_UNIT,*) 'DIRECT2D: Results in C matrix'
