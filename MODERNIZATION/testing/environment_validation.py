...
class EnvironmentValidator:
    ...
    def __init__(self):
        """Initialize the environment validator with default settings."""
    ...
    def validate_phase_1a(self) -> Dict:
        ...
        if self.is_container:
            ...
            for dep_name, test_code in deps_to_test:
                dep_success, dep_output = self.run_command(["python", "-c", test_code])
    ...
    def validate_phase_1b(self) -> Dict:
        ...
        for doc_path in analysis_docs:
            ...
            results[f"document_{doc_name}"] = {
                ...
                "details": (
                    f"File size: {doc_file.stat().st_size if exists else 0} bytes"
                ),
                "required": True,
            }
            ...
            if exists:
                ...
                self.log(f"{doc_name}: Present ({size} bytes)", "PASS")
            else:
                self.log(f"{doc_name}: Missing", "FAIL")

        # Validate LAPACK source files accessibility
        lapack_files = ["SRC/dgesvd.f", "BLAS/SRC/dgemm.f", "LAPACKE/src"]
    ...
    def validate_phase_1c(self) -> Dict:
        ...
        if self.is_container:
            ...
            for cmd, tool_name in build_tools:
                ...
                tool_key = f"build_tool_{tool_name.lower().replace(' ', '_')}"
                results[tool_key] = {
...
""",
    ...
    def validate_documentation(self) -> Dict:
        ...
        required_docs = [
            (
                "MODERNIZATION/_docs/phase1_implementation_plan.md",
                "Implementation plan",
            ),
            ...
            (
                "MODERNIZATION/codebase-research/dev_environment_setup.md",
                "Environment setup",
            ),
    ...
    def validate_build_system(self) -> Dict:
        ...
        if not self.is_container:
            ...
            return {
                "build_system_skipped": {
                    "status": "WARN",
                    "details": "Not in container",
                    "required": False,
                }
            }

        # Test CMake configuration
        cmake_success, cmake_output = self.run_command(["cmake", "--version"])
...
def main():
    """Run main validation entry point."""
