...
*
*  -- Ultimate Multi-Size Matrix AlphaTensor Testing Framework --
*  -- Tests: All 3 algorithms across multiple matrix sizes --
*  -- Sizes: 4x4 (AlphaTensor), 8x8, 16x16, 32x32 (fallback) --
*  -- Coverage: 12 matrix types, speed, performance, accuracy --
*  -- Output: Detailed report showing AlphaTensor active vs fallback --
*
      IMPLICIT NONE
*
*     .. Parameters ..
...
*     .. Local Arrays ..
      ...
      DOUBLE PRECISION C_OPT(32,32)
      DOUBLE PRECISION INITIAL_C(32,32)
      CHARACTER*50 TEST_NAMES(12)
      CHARACTER*30 ALGO_NAMES(3)
      INTEGER MATRIX_SIZES(4), NRUNS_BY_SIZE(4)
      DOUBLE PRECISION TIMES(3), GFLOPS(3)
      INTEGER ALGO_RANKINGS(3)
*     ..
*     .. Local Scalars ..
      ...
      DOUBLE PRECISION FLOPS_CURRENT, SPEEDUP_ALPHA, SPEEDUP_OPT
...
*     Open comprehensive report file
     ...
     $     FILE='ultimate_multi_size_alphatensor_report.txt',
     $     STATUS='REPLACE')
...
*     Write detailed report header
     ...
     +  '============================================================'
      WRITE(REPORT_UNIT, '(A)')
     +  '   ULTIMATE MULTI-SIZE ALPHATENSOR TESTING FRAMEWORK'
      WRITE(REPORT_UNIT, '(A)')
     +  '============================================================'
...
*     Console header
      WRITE(*,*) '=============================================='
      WRITE(*,*) '   ULTIMATE MULTI-SIZE ALPHATENSOR TESTING'
      WRITE(*,*) '=============================================='
      WRITE(*,*) 'Testing 3 algorithms across', NUM_SIZES,
     +           'matrix sizes'
...
*             Precise timed benchmark - DGEMM_ALPHA
     ...
     +            SPEEDUP_ALPHA, 'x'
              ...
              ELSE
                  WRITE(REPORT_UNIT, '(A)') '  (Fallback + overhead)'
              END IF

*             ========================================================
*             BENCHMARK 3: DGEMM_ALPHA_OPTIMIZED (Performance Optimized)
*             ========================================================
              WRITE(REPORT_UNIT, '(A)') 'ALGORITHM 3: DGEMM_ALPHA_OPT'
...
*             Warm-up runs
              DO RUN = 1, MIN(1000, CURRENT_NRUNS/10)
                  CALL DGEMM_ALPHA_OPTIMIZED('N','N',CURRENT_SIZE,
     +                CURRENT_SIZE,CURRENT_SIZE,ALPHA,A,32,
     +                B,32,BETA,C_OPT,32)
          END DO
...
*             Precise timed benchmark - DGEMM_ALPHA_OPTIMIZED
     ...
     +                B,32,BETA,C_OPT,32)
          ...
          CALL CPU_TIME(END_TIME)
              ...
              GFLOPS(3) = (FLOPS_CURRENT * DBLE(CURRENT_NRUNS)) /
     +                    (TIMES(3) * 1.0D+9)
...
*             DGEMM_ALPHA_OPTIMIZED vs DGEMM accuracy
          ...
          ERROR_SUM = ZERO
              DO J = 1, CURRENT_SIZE
                  ...
                  END DO
              END DO
              AVG_O = ERROR_SUM / DBLE(CURRENT_SIZE * CURRENT_SIZE)
...
*             Sort algorithms by performance (simple ranking)
              DO I = 1, NUM_ALGOS
                  ALGO_RANKINGS(I) = I
              END DO
              IF (TIMES(2) .LT. TIMES(1)) THEN
                  IF (TIMES(2) .LT. TIMES(3)) THEN
                      ALGO_RANKINGS(1) = 2
     ...
     $            MAX_A .LT. 1.0D-10) THEN
              ...
              ELSE
                  ...
                  IF (SPEEDUP_ALPHA .LE. 1.0D+0) THEN
                      WRITE(REPORT_UNIT, '(A)') '    - Perf below'
                  END IF
...
*     ============================================================
     ...
     +    PASSED_TESTS_OPT, ' / ', TOTAL_TESTS_RUN
      WRITE(REPORT_UNIT, '(A,I0,A,I0)') 'DGEMM_OPT Tests Failed: ',
     +    FAILED_TESTS_OPT, ' / ', TOTAL_TESTS_RUN
      WRITE(REPORT_UNIT, '(A,F8.3,A)') 'DGEMM_OPT Speedup: ',
     +    TOTAL_SPEEDUP_OPT / DBLE(TOTAL_TESTS_RUN), 'x'
      WRITE(REPORT_UNIT, '(A,ES15.8)') 'DGEMM_OPT Max Error: ',
     +    TOTAL_ACCURACY_OPT / DBLE(TOTAL_TESTS_RUN)
      ...
      WRITE(REPORT_UNIT, '(A)') '- AlphaTensor effectiveness on target'
      WRITE(REPORT_UNIT, '(A)') '- Fallback overhead vs pure DGEMM'
      WRITE(REPORT_UNIT, '(A)') '- Scaling behavior with matrix size'

      WRITE(REPORT_UNIT, '(A)') ''
      WRITE(REPORT_UNIT, '(A)') 'RECOMMENDATION:'
      IF (PASSED_TESTS_OPT .GE. TOTAL_TESTS_RUN * 3 / 4) THEN
          WRITE(REPORT_UNIT, '(A)')
     +      'GOOD: AlphaTensor shows promise for target cases.'
      ELSE IF (PASSED_TESTS_ALPHA .GE. TOTAL_TESTS_RUN / 2) THEN
          WRITE(REPORT_UNIT, '(A)')
     +      'MIXED: Some benefits visible, needs optimization.'
      ELSE
          WRITE(REPORT_UNIT, '(A)')
     +      'CAUTION: Stick with DGEMM until issues resolved.'
      ...
      WRITE(REPORT_UNIT, '(A)') '1. Focus on 4x4 performance'
      WRITE(REPORT_UNIT, '(A)') '2. Reduce fallback overhead'
      WRITE(REPORT_UNIT, '(A)') '3. Matrix-size-specific optimizations'
      WRITE(REPORT_UNIT, '(A)') '4. Consider blocking for larger sizes'
      WRITE(REPORT_UNIT, '(A)')
     +  '============================================================'
...
*     Final console summary
      WRITE(*,*) '=============================================='
      WRITE(*,*) 'ULTIMATE MULTI-SIZE TESTING COMPLETE!'
     ...
     +    TOTAL_SPEEDUP_ALPHA / DBLE(TOTAL_TESTS_RUN), 'x, OPT=',
     +    TOTAL_SPEEDUP_OPT / DBLE(TOTAL_TESTS_RUN), 'x'
      WRITE(*,*) 'Report: ultimate_multi_size_alphatensor_report.txt'
      WRITE(*,*) '=============================================='
