#!/usr/bin/env python3
"""LAPACK Testing Script.

This script analyzes LAPACK test results and provides comprehensive summaries
of test outcomes including numerical errors and other failures.
"""
...
for o, a in opts:
    if o in ("-h", "--help"):
        help_msg = (
            f"{sys.argv[0]} [-h|--help] [-d dir |--dir dir] [-s |--short] "
            "[-r |--run] [-e |--error] [-p p |--prec p] [-t test |--test test] "
            "[-n | --number]"
        )
        print(help_msg)
        print("     - h is to print this message")
        print(
            "     - r is to use to run the LAPACK tests then analyse the output "
            "(.out files). By default, the script will not run all the LAPACK tests"
        )
        dir_msg = (
            f"     - d [dir] indicates the location of the LAPACK testing directory "
            f"(.out files). By default, the script will use {test_dir}."
        )
        print(dir_msg)
        bin_msg = (
            f"     - b [bin] indicates the location of the LAPACK binary files. "
            f"By default, the script will use {bin_dir}."
        )
        print(bin_msg)
        ...
        print(
            "            Will return the numbers of failed tests by analyzing "
            "the LAPACK output"
        )
        print("     ./lapack_testing.py -n -r -p s")
        print(
            "            Will return the numbers of failed tests in REAL precision "
            "by running the LAPACK Tests then analyzing the output"
        )
        print("     ./lapack_testing.py -n -p s -t eig ")
        print(
            "            Will return the numbers of failed tests in REAL precision "
            "by analyzing only the LAPACK output of EIGEN testings"
        )
        sys.exit(0)
    else:
        if o in ("-s", "--short"):
...
if with_file:
    summary += (
        f"\t\tProcessing LAPACK Testing output found in the " f"{test_dir} directory\n"
    )
...
def run_summary_test(f, cmdline, short_summary):
    """Execute and analyze a LAPACK test, returning test statistics.

    Args:
        f: File handle for writing detailed output
        cmdline: Command line to execute or output file to read
        short_summary: Boolean flag for short summary mode

    Returns:
        List containing [nb_test_run, nb_test_fail, nb_test_illegal, nb_test_info]
    """
    nb_test_run = 0
    nb_test_fail = 0
    nb_test_illegal = 0
    ...
    if r != 0 and not with_file:
        print(
            "---- TESTING " + cmdline.split()[0] + "... FAILED(" + error_message + ") !"
        )
        for line in pipe.readlines():
            f.write(str(line))
    elif r != 0 and with_file and not short_summary:
        print(
            "---- WARNING: please check that you have the LAPACK output : "
            + cmdline
            + "!"
        )
...
for dtype in range_prec:
    ...
    dtests = (
        ...
        (
            ...
            "Nonsymmetric-Generalized-Eigenvalue-Problem-driver",
            "Symmetric-Eigenvalue-Problem",
    ...
    for dtest in range_test:
        ...
        if not short_summary:
            ...
            if not just_errors:
                print("")
        #     elif (got_error>0):
        #        print (dtests[2][dtest]+".out \t"+str(nb_test[1])+
        #               "\t"+str(nb_test[2])+"\t"+str(nb_test[3]))

        sys.stdout.flush()
    if list_results[0][dtype] > 0:
        percent_num_error = (
            float(list_results[1][dtype]) / float(list_results[0][dtype]) * 100
        )
        percent_error = (
            float(list_results[2][dtype] + list_results[3][dtype])
            / float(list_results[0][dtype])
            * 100
        )
    else:
        percent_num_error = 0
...
if only_numbers:
    ...
    if list_results[0][4] > 0:
        ...
        percent_error = (
            float(list_results[2][4] + list_results[3][4])
            / float(list_results[0][4])
            * 100
        )
    else:
        percent_num_error = 0
        percent_error = 0
    if prec == "x":
        all_precision_summary = (
            "--> ALL PRECISIONS\t"
            + str(list_results[0][4])
            + "\t\t"
